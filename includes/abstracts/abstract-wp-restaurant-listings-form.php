...
abstract class WP_Restaurant_Listings_Form {
	...
	public function process() {
		...
		if (
			isset( $_GET[ 'new' ] ) &&
			isset( $_COOKIE[ 'wp-restaurant-listings-submitting-restaurant-id' ] ) &&
			isset( $_COOKIE[ 'wp-restaurant-listings-submitting-restaurant-key' ] ) &&
	...
	public function show_errors() {
		foreach ( $this->errors as $error ) {
			echo '<div class="restaurant-listings-error">' . $error . '</div>';
		}
	}
	...
	protected function get_posted_fields() {
		...
		foreach ( $this->fields as $group_key => $group_fields ) {
			foreach ( $group_fields as $key => $field ) {
				...
				if ( $handler = apply_filters( "restaurant_listings_get_posted_{$field_type}_field", false ) ) {
	 ...
	 *
	 * @since 1.0.0
	 * @since 1.0.1 Added the $sanitizer argument
	 *
	...
	protected function upload_file( $field_key, $field ) {
		if ( isset( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ]['name'] ) ) {
			if ( ! empty( $field['allowed_mime_types'] ) ) {
				...
				$allowed_mime_types = restaurant_listings_get_allowed_mime_types();
			}

			$file_urls       = array();
			$files_to_upload = restaurant_listings_prepare_uploaded_files( $_FILES[ $field_key ] );
